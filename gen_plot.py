...
for idx, (name, df) in enumerate(zip(names, dfs)):
    ...
    df["smooth_temp_celsius"] = df["temp_celsius"].ewm(span=5, adjust=True).mean()
    ...
    T_init = df['smooth_temp_celsius'].iloc[0]
    T_ss = df['smooth_temp_celsius'].iloc[-1]
    T_ss = 35
    ...
    temp_plot.plot(x, y, label="two-exp regression")
